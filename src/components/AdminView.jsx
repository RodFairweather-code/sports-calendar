...
function buildDefaultTimingsSnapshot() {
  const timings = JSON.parse(localStorage.getItem('admin_default_timings') || '{}')
  const date    = new Date().toISOString().slice(0, 10)

  const lines = []
  lines.push(`// Sports Calendar — Default Timings Seed Data`)
  lines.push(`// Generated by the Snapshot Default Timings button in Admin.`)
  lines.push(`// To update: press Snapshot Default Timings, replace this file with the downloaded seedDefaultTimings.js, then commit.`)
  lines.push(`// Applied on first load only when localStorage has no default-timings data.`)
  lines.push(`// Last snapshot: ${date}`)
  lines.push(``)
  lines.push(`export const SEED_DEFAULT_TIMINGS = ${JSON.stringify(timings, null, 2)}`)
  return lines.join('\n')
}
...
function AdminView({
  ...
  function handleSnapshotDefaultTimings() {
    downloadText('seedDefaultTimings.js', buildDefaultTimingsSnapshot())
  }
      ...
      <div className="admin-bottom-bar">
        ...
        <div className="admin-snapshot-group">
          ...
          <button
            ...
            disabled={!snapshotUnlocked}
            tabIndex={snapshotUnlocked ? 0 : -1}
            onClick={handleSnapshotDefaultTimings}
          >
            Snapshot Default Timings
          </button>
          <button
            className={`admin-tab-btn admin-snapshot-btn${snapshotUnlocked ? ' visible' : ''}`}
            disabled={!snapshotUnlocked}
